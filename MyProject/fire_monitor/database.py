...
class ImageDatabase:
    ...
    def search_images(self, query=None, category=None, page=1, page_size=20):
        """搜索图像并支持分页
        Args:
            query: 搜索关键词(文件名)
            category: 图像类别(fire/non_fire)
            page: 页码(从1开始)
            page_size: 每页显示数量
        Returns:
            图像列表和总页数
        ...
        try:
            ...
            # 基础查询
            base_query = "SELECT id, filename, timestamp, category, prediction FROM images"
            if conditions:
            ...
            self.cursor.execute(paginated_query, params)

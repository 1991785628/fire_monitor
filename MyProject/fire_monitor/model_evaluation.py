...
# 创建train_chart文件夹
os.makedirs('eval_chart', exist_ok=True)
...
def visualize_predictions(model, test_generator, fire_samples=5, non_fire_samples=4):
    ...
    plt.tight_layout()
    plt.savefig('eval_chart/predictions.png')
    plt.close()
    print('预测结果可视化完成，已保存为predictions.png')

def evaluate_model(model_path, image_size = (224, 224), batch_size = 1, multimodal=False):
    #创建数据生成器
    if multimodal:
        _, _, test_generator = create_multimodal_generators(
    ...
    plt.ylabel('真实标签')
    plt.title('混淆矩阵')
    plt.savefig('eval_chart/confusion_matrix.png')
    plt.close()
    ...
    plt.savefig('eval_chart/roc_curve.png')
    plt.close()
    ...
    plt.grid = True
    plt.tight_layout()
    plt.savefig('eval_chart/precision_recall_curve.png')
    plt.close()
...
if __name__ == '__main__':
    # 解析命令行参数
    parser = argparse.ArgumentParser(description='评估火灾检测模型')
    parser.add_argument('--model_path', type=str, default='models/final_model.keras', help='模型路径')
    parser.add_argument('--image_size', type=int, nargs=2, default=(224, 224), help='图像大小 (宽度 高度)')
    parser.add_argument('--batch_size', type=int, default=1, help='批次大小')
    parser.add_argument('--multimodal', action='store_true', help='是否使用多模态模型评估')
